...
function RECIPE_ADD_CUSTOM_TOOLTIP_TEXT(invItem)
    ...
    local count = 0;
    local partOfRecipePrimaryTable = {};
    local partOfRecipeSubTable = {};
    for _, textStr in pairs(partOfRecipe) do
        if count < 25 then
            table.insert(partOfRecipePrimaryTable, textStr);
        else 
            table.insert(partOfRecipeSubTable, textStr);
        end
        count = count + 1 
    end
    local primaryText = table.concat(partOfRecipePrimaryTable, "{nl}");
    local secondaryText = table.concat(partOfRecipeSubTable, "{nl}");
...
function RENDER_RECIPE_DETAILS(tooltipFrame, primaryBuffer, secondaryBuffer, invItem, primaryText, secondaryText)
    if TooltipHelper.config.showRecipeCustomTooltips then 
        primaryText, secondaryText = RECIPE_ADD_CUSTOM_TOOLTIP_TEXT(invItem)
        if primaryText ~= "" and secondaryText == "" then
            table.insert(primaryBuffer, primaryText)
        elseif primaryText ~= "" and secondaryText ~= "" then
            table.insert(primaryBuffer, primaryText)
            table.insert(secondaryBuffer, secondaryText)
        end
    end
end
...
function CUSTOM_TOOLTIP_PROPS(tooltipFrame, mainFrameName, invItem, strArg, useSubFrame)
    ...
    local primaryTextCtrl = gBox:CreateOrGetControl("richtext", 'primaryText', 0, yPos, 450, 30);
    tolua.cast(primaryTextCtrl, "ui::CRichText");
    ...
    local secondaryText = "";
    local recipeSubBuffer = {};
    --Recipe
    RENDER_RECIPE_DETAILS(tooltipFrame, buffer, recipeSubBuffer, invItem, text, secondaryText)
   
    local magnumOpusText = ""
    local magnumOpusBuffer = {}
    --Magnum Opus
    RENDER_MAGNUM_OPUS(tooltipFrame, magnumOpusBuffer, invItem, magnumOpusText)
    ...
    if #buffer == 1 and invItem.ItemType == "Equip" then
        ...
        secondaryText = table.concat(recipeSubBuffer,"{nl}") .. table.concat(magnumOpusBuffer,"{nl}")
    end
        
    primaryTextCtrl:SetText(text);
    primaryTextCtrl:SetMargin(20,gBox:GetHeight() - 10,0,0)
    primaryTextCtrl:SetGravity(ui.LEFT, ui.TOP)
    
    if secondaryText ~= "" then
        local secondaryTextCtrl = gBox:CreateOrGetControl("richtext", 'secondaryText', primaryTextCtrl:GetX() + 50, yPos, primaryTextCtrl:GetWidth(), primaryTextCtrl:GetHeight());
        tolua.cast(secondaryTextCtrl, "ui::CRichText");
        secondaryTextCtrl:SetText(secondaryText)
        secondaryTextCtrl:SetMargin(0, gBox:GetHeight() - 10,0,0)
        secondaryTextCtrl:SetGravity(ui.RIGHT, ui.TOP)
        
        local width = primaryTextCtrl:GetWidth() + secondaryTextCtrl:GetWidth() + 100;
        if primaryTextCtrl:GetHeight() > secondaryTextCtrl:GetHeight() then
            gBox:Resize(width, gBox:GetHeight() + primaryTextCtrl:GetHeight())      
        else 
            gBox:Resize(width, gBox:GetHeight() + secondaryTextCtrl:GetHeight())
        end
        ...
        gBox:Resize(gBox:GetWidth(), gBox:GetHeight() + primaryTextCtrl:GetHeight())    
    end
